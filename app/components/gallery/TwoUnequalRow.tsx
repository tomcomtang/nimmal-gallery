...
interface TwoUnequalRowProps {
  photos: Photo[]
  onPhotoClick: (photo: Photo, e: React.MouseEvent) => void
}

export default function TwoUnequalRow({ photos, onPhotoClick }: TwoUnequalRowProps) {
  return (
    <div className="md:col-span-12 grid grid-cols-1 md:grid-cols-12 gap-8">
      {photos.map((photo, index) => (
        <div 
          key={photo.id}
          className={`md:col-span-${index === 0 ? '5' : '7'} group relative bg-white rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer`}
